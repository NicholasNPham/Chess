...
class GameState():
    def __init__(self):
        # Board is a 8x8 2-D List, each element of the list has 2 characters.
        # The first Character represents the color of the piece, 'b' or 'w'.
        # The Second character represents the type of piece, K, Q, R, B, N, and P.
        ...
        self.castleRightsLog = [CastleRights(self.currentCastlingRight.wks, self.currentCastlingRight.bks,
                                             self.currentCastlingRight.wqs, self.currentCastlingRight.wqs)]
...
# Takes a Move as a parameter and executes it (This will not work for castling, pawn promotion and en-passant)
    def makeMove(self, move):
        ...
        if move.pieceMoved[1] == 'p' and abs(move.startRow - move.endRow) == 2: # Only one 2 square pawn advances
    ...
    def getValidMoves(self):
        ...
        for i in range(len(moves)-1, -1, -1): # When removing from a list go backwards through that list.
            ...
            self.whiteToMove = not self.whiteToMove
            self.undoMove()
        if len(moves) == 0: # Check if either checkmate or stalemate
            if self.inCheck():
                self.checkmate = True
            else:
                self.stalemate = True
    ...
    # Determine if the current play is in check
    def inCheck(self):
        if self.whiteToMove:
    ...
    def getAllPossibleMoves(self):
        ...
        for r in range(len(self.board)):  # Number of rows
            for c in range(len(self.board[r])): # Numbers of cols in given rows
...
class Move():
    ...
    def __str__(self):
        ...
        # Two of the same type of piece moving to a square, Nbd2 if both knights cam move to d2.

        # Also adding + for checkmove, and # for checkmate move.
